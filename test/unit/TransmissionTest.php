...
class TransmissionTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->sparkPostMock = Mockery::mock(
            'SparkPost\SparkPost',
            function ($mock) {
                $mock->shouldReceive('getHttpHeaders')->andReturn(array());
            }
        );
    ...
    public function testSend()
    {
        ...
        $body = array('text' => 'awesomesauce', 'content' => array('subject' => 'awesomeness'));
        $responseBody = array('results' => 'yay');

        $this->sparkPostMock->httpAdapter
            ->shouldReceive('send')
            ->once()
